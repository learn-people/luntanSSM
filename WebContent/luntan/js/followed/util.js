...
//页面加载完成以后，直接发送ajax请求，拿到分页数据
 ...
 function to_page(pn){
   $.ajax({
	 url:"/luntanSSM/followed/selectAll.do",
     data:"pn="+pn,
     type:"GET",
     success:function(result){
       console.log(result);
       //1、解析并显示员工数据
       build_followed_advert(result);
       //2、解析并显示分页信息
 ...
 * 
 */
 function build_followed_advert(result) {
	 //清空table表格,如果不请空，下一次请求时，数据会在原来的基础上，不断添加。
	 $("#followed_advert tbody").empty();
	 var followed = result.extend.pageInfo.list;
	
	 $.each(followed, function(index, item) {
		 //alert(item.empName);
		 //构建单元格
		 var checkBoxTd = $("<td><input type='checkbox' class='check-item' /></td>");
		 var followedIdTd = $("<td></td>").append(item.id);
		 var followedUserIdTd = $("<td></td>").append(item.followedUserId);
		 ...
		 //设置id属性为不可见
		 followedIdTd.attr("style","display:none");
		 ...
		 //.append(deleteBtn)
		 .append(btnTd)
		 .appendTo("#followed_advert tbody");
