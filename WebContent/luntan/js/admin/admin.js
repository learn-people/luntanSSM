...
//校验账号是否可用，需要在保存按钮提交前进行判断，自定义属性留个记号，让保存按钮可用来判断后端校验情况。
 $("#adminNumber_add_input").change(function(){
	 if(!validate_form_ele("#adminNumber_add_input",g_adminNumber_reg,g_adminNumber_valid,g_adminName_invalid_exist)){
		 return false;
	 } 
	 //发送ajax请求，校验用户名是否可用
	 var adminNumber = this.value;
	 $.ajax({
		 url:"/luntanSSM/admin/checkAdminNumber.do",
		 type:"POST",
		 data:"adminNumber=" + adminNumber,
		 success:function(result){
			 if(result.code == 100){
				 show_validate_msg("#adminNumber_add_input","success",g_adminNumber_valid);
				 $("#admin_add_save_btn").attr("ajax-check-adminNumber","success");
			 }else{
				 show_validate_msg("#adminNumber_add_input","error",result.extend.adminNumber_msg)
				 $("#admin_add_save_btn").attr("ajax-check-adminNumber","error"); 
			 }
		 }
	 });
 });
 ...
 $("#adminName_add_input").change(function(){
	 if(!validate_form_ele("#adminName_add_input",g_adminName_reg,g_adminName_valid,g_adminName_invalid_exist)){
	   return false;
	 } 
	 //发送ajax请求，校验用户名是否可用
	 var adminName = this.value;
	 $.ajax({
		 url:"/luntanSSM/admin/checkAdminName.do",
		 type:"POST",
		 data:"adminName=" + adminName,
		 success:function(result){
			 if(result.code == 100){
				 show_validate_msg("#adminName_add_input","success",g_adminName_valid);
				 $("#admin_add_save_btn").attr("ajax-check-adminName","success");
			 }else{
				 show_validate_msg("#adminName_add_input","error",result.extend.adminName_msg)
				 $("#admin_add_save_btn").attr("ajax-check-adminName","error"); 
			 }
		 }
	 });
 });
...
//添加模态框 点击保存按钮的事件
 $("#admin_add_save_btn").click(function(){
	 ...
	 if($(this).attr("ajax-check-adminName") == "error"){
		 return false;
	 }
	 if($(this).attr("ajax-check-adminNumber") == "error"){
		 return false;
	 }
	 
	 if(!validate_form_ele("#adminName_add_input",g_adminName_reg,g_adminName_valid,g_adminName_invalid_format)) {
		 return false;		 
	 }
		 
	 if(!validate_form_ele("#adminNumber_add_input",g_adminNumber_reg,g_adminNumber_valid,g_adminNumber_invalid_format)){
		 return false;		 
	 } 
	 //查看是否添加了图片
	 if ($("#add_imgName").val()) {
	 ...
	 $.ajax({
		 ...
		 success:function(result){
			 ...
			 if(result.code == 100){
				 ...
				 //console.log(result);
				 //有哪个字段的错误信息，就显示哪个字段
				 //alert(result.extend.errorFileds.adminNumber);
				 //alert(result.extend.errorFileds.adminName);
				 if(undefined!=result.extend.errorFileds.adminNumber){
					 //显示账号错误信息
					 show_validate_msg("#adminNumber_add_input","error",result.extend.errorFileds.adminNumber);
				 }
				 
				 if(undefined!=result.extend.errorFileds.adminName){
					 //显示员工名字错误信息
					 show_validate_msg("#adminName_add_input","error",result.extend.errorFileds.adminName);
				 }
			 }
			 //调用清空表单的方法
