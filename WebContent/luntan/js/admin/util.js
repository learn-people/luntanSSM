  ...
  * 换表时要更改 4-13行的名称
  */
 var g_adminName_reg = /(^[a-zA-Z0-9_-]{5,16}$)|(^[\u2E80-\u9FFF]{2,10})/;
 var g_adminName_valid = "用户名可用";
 var g_adminName_invalid_format = "用户名可以是2-10位的中文或者5-16位英文和数字";
 var g_adminName_invalid_exist = "用户名已存在";
 
 var g_adminNumber_reg = /^\d{3,11}$/;
 var g_adminNumber_valid = "账号可用";
 var g_adminNumber_invalid_format = "账号必须是3-11位的数字";
 var g_adminNumber_invalid_exist = "账号已存在";
...
//页面加载完成以后，直接发送ajax请求，拿到分页数据
 ...
 function to_page(pn){
   $.ajax({
	 url:"/luntanSSM/admin/selectAll.do",
     data:"pn="+pn,
     type:"GET",
 ...
 * 
 */
 function build_admin_advert(result) {
	 //清空table表格,如果不请空，下一次请求时，数据会在原来的基础上，不断添加。
	 $("#admin_advert tbody").empty();
	 ...
	 $.each(admin, function(index, item) {
		 ...
		 var adminBirthdayTd = $("<td></td>").append(item.adminBirthday);
		 var imgUrlTd = $("<td></td>").append(item.imgUrl);
		 var adminGradeTd = $("<td></td>").append(item.adminGrade); 
		 var positionTd = $("<td></td>").append(item.position); 
		 var adminHandleNumTd = $("<td></td>").append(item.adminHandleNum);
		 //设置id属性为不可见
		 adminIdTd.attr("style","display:none");
		 ...
		 .append(adminIdTd)
		 .append(adminNameTd)
		 .append(adminBirthdayTd)
		 .append(imgUrlTd)
		 .append(adminGradeTd)
		 .append(positionTd)
		 .append(adminHandleNumTd)
		 //.append(editBtn)
		 //.append(deleteBtn)
		 .append(btnTd)
		 .appendTo("#admin_advert tbody");
