...
$(document).on("click",".edit-btn",function(){
	 ...
	 $("#admin_update_done_btn").attr("edit-id",$(this).attr("edit-id"));
	 $("#adminUpdateModal").dialog("open");
	 //alert("edit");
 ...
 $("#adminName_update_input").change(function(){
	 if(!validate_form_ele("#adminName_update_input",g_adminName_reg,g_adminName_valid,g_adminName_invalid_exist)){
	   return false;
	 } 
	 //发送ajax请求，校验用户名是否可用
	 ...
	 $.ajax({
		 ...
		 success:function(result){
			 if(result.code == 100){
				 show_validate_msg("#adminName_update_input","success",g_adminName_valid);
				 $("#admin_update_done_btn").attr("ajax-check-adminName","success");
			 }else{
				 show_validate_msg("#adminName_update_input","error",result.extend.adminName_msg)
				 $("#admin_update_done_btn").attr("ajax-check-adminName","error"); 
			 }
		 }
	 });
 });
 ...
 function getAdmin(id){
   $.ajax({
	 url:"/luntanSSM/admin/get.do?id="+id,
	 type:"get",
	 success:function(result){
	   console.log(result);
	   var adminData = result.extend.admin;
	   $("#adminNumber_update_input").val(adminData.adminNumber);
	   $("#adminPassword_update_input").val(adminData.adminPassword);
	   $("#adminName_update_input").val(adminData.adminName);
	   $("#adminBirthday_update_input").val(adminData.adminBirthday);
	   $("#position_update_input").val(adminData.position);
	   $("#update_imgUrl").val(adminData.imgUrl);
	   update_imgName = adminData.update_imgName;
	 }
...
//点击更新，更新员工信息
 $("#admin_update_done_btn").click(function(){
	//判断之前的ajax用户名和账号重复性校验是否成功
	 if($(this).attr("ajax-check-adminName") == "error"){
		 return false;
	 }
 	//验证用户名是否合法
 	if(!validate_form_ele("#adminName_update_input",g_adminName_reg,g_adminName_valid,g_adminName_invalid_format)){
         return false;
 	}
 	//查看是否添加了图片
 ...
 function saveData(){
	 $.ajax({
		 url:"/luntanSSM/admin/update.do?id="+$("#admin_update_done_btn").attr("edit-id"),
		 type:"post",
		 data:$("#adminUpdateModal form").serialize()+"&_method=put",
		 success:function(result){
			 //alert(result);
			 //console.log(result);
			 if(result.code=100){
				 //1、员工修改成功，需要关闭模态框。
				 $("#adminUpdateModal").dialog("close"); 
				 ...
				 if(undefined!=result.extend.errorFileds.email){
					 //显示用户名错误信息
					 show_validate_msg("#adminName_update_input","error",result.extend.errorFileds.adminName);
				 }
			 }		
		 }//success 方法结束
	 });
