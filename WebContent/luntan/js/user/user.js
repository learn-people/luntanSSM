 ...
 * */
 $("#user_add").click(function(){
  $("#userAddModal").dialog("open");
 });
 
 
//校验账号是否可用，需要在保存按钮提交前进行判断，自定义属性留个记号，让保存按钮可用来判断后端校验情况。
 $("#userNumber_add_input").change(function(){
	 if(!validate_form_ele("#userNumber_add_input",g_userNumber_reg,g_userNumber_valid,g_userName_invalid_exist)){
		 return false;
	 } 
	 //发送ajax请求，校验用户名是否可用
	 var userNumber = this.value;
	 $.ajax({
		 url:"/luntanSSM/user/checkUserNumber.do",
		 type:"POST",
		 data:"userNumber=" + userNumber,
		 success:function(result){
			 if(result.code == 100){
				 show_validate_msg("#userNumber_add_input","success",g_userNumber_valid);
				 $("#user_add_save_btn").attr("ajax-check-userNumber","success");
			 }else{
				 show_validate_msg("#userNumber_add_input","error",result.extend.userNumber_msg)
				 $("#user_add_save_btn").attr("ajax-check-userNumber","error"); 
			 }
		 }
	 });
 });
 ...
 $("#userName_add_input").change(function(){
	 if(!validate_form_ele("#userName_add_input",g_userName_reg,g_userName_valid,g_userName_invalid_exist)){
	   return false;
	 } 
	 //发送ajax请求，校验用户名是否可用
	 ...
	 $.ajax({
		 ...
		 success:function(result){
			 if(result.code == 100){
				 show_validate_msg("#userName_add_input","success",g_userName_valid);
				 $("#user_add_save_btn").attr("ajax-check-userName","success");
			 }else{
				 show_validate_msg("#userName_add_input","error",result.extend.userName_msg)
				 $("#user_add_save_btn").attr("ajax-check-userName","error"); 
			 }
		 }
	 });
 });
...
//添加模态框 点击保存按钮的事件
 $("#user_add_save_btn").click(function(){
 	 //判断之前的ajax用户名和账号重复性校验是否成功
	 if($(this).attr("ajax-check-userName") == "error"){
		 return false;
	 }
	 if($(this).attr("ajax-check-userNumber") == "error"){
		 return false;
	 }
	 
	 if(!validate_form_ele("#userName_add_input",g_userName_reg,g_userName_valid,g_userName_invalid_format)) {
		 return false;		 
	 }
		 
	 if(!validate_form_ele("#userNumber_add_input",g_userNumber_reg,g_userNumber_valid,g_userNumber_invalid_format)){
		 return false;		 
	 } 
	 //查看是否添加了图片
	 if ($("#add_imgName").val()) {
	 ...
	 $.ajax({
		 ...
		 success:function(result){
			 ...
			 if(result.code == 100){
				 ...
				 //console.log(result);
				 //有哪个字段的错误信息，就显示哪个字段
				 //alert(result.extend.errorFileds.userNumber);
				 //alert(result.extend.errorFileds.userName);
				 if(undefined!=result.extend.errorFileds.userNumber){
					 //显示账号错误信息
					 show_validate_msg("#userNumber_add_input","error",result.extend.errorFileds.userNumber);
				 }
				 
				 if(undefined!=result.extend.errorFileds.userName){
					 //显示员工名字错误信息
					 show_validate_msg("#userName_add_input","error",result.extend.errorFileds.userName);
				 }
			 }
			 //调用清空表单的方法
