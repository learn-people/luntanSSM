...
//页面加载完成以后，直接发送ajax请求，拿到分页数据
 ...
 function to_page(pn){
   $.ajax({
	 url:"/luntanSSM/userdetail/selectAll.do",
     data:"pn="+pn,
     type:"GET",
     ...
     //console.log(result);
     //1、解析并显示员工数据
     build_userdetail_advert(result);
     //2、解析并显示分页信息
 ...
 * 
 */
 function build_userdetail_advert(result) {
	 //清空table表格,如果不请空，下一次请求时，数据会在原来的基础上，不断添加。
	 $("#userdetail_advert tbody").empty();
	 var userdetail = result.extend.pageInfo.list;
	
	 $.each(userdetail, function(index, item) {
		 //alert(item.empName);
		 //构建单元格
		 ...
		 var hometownTd = $("<td></td>").append(item.hometown); 
		 //设置id属性为不可见
		 ...
		 //.append(deleteBtn)
		 .append(btnTd)
		 .appendTo("#userdetail_advert tbody");
