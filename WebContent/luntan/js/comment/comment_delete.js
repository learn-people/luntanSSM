/****************************删除信息********************************************************************************************/
 ...
 $(document).on("click",".delete-btn",function(){
   ...
   var postId= $(this).parents("tr").find("td:eq(1)").text()
   var id = $(this).attr("delete-id");
   if(confirm("确认删除【"+postId+"】")){
     $.ajax({
       url:"/luntanSSM/comment/deleteOne.do?id="+id,
       type:"delete",
...
//完成全选、全不选功能
 ...
 $("#comment_delete_all_btn").click(function(){			
 	...
 	$.each($(".check-item:checked"),function(){
 		postId = postId+$(this).parents("tr").find("td:eq(2)").text() +",";
 	...
 	//发送ajax请求
 	  $.ajax({
		url:"/luntanSSM/comment/delete.do?ids="+delete_ids_str,
		type:"delete",
		success:function(result){
			alert(result.msg);
			to_page(currentPage);
