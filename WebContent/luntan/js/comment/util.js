...
//页面加载完成以后，直接发送ajax请求，拿到分页数据
 ...
 function to_page(pn){
   $.ajax({
	 url:"/luntanSSM/comment/selectAll.do",
     data:"pn="+pn,
     type:"GET",
     success:function(result){
    	 console.log(result);
 ...
 * 
 */
 function build_comment_advert(result) {
	 //清空table表格,如果不请空，下一次请求时，数据会在原来的基础上，不断添加。
	 $("#comment_advert tbody").empty();
	 var comment = result.extend.pageInfo.list;
	 ...
	 $.each(comment, function(index, item) {
		 ...
		 .append(userNameTd)
		 .append(userImgUrlTd)
		 .append(commentContentTd)
		 .append(imgUrlTd)
		 //.append(editBtn)
		 //.append(deleteBtn)
		 .append(btnTd)
		 .appendTo("#comment_advert tbody");
