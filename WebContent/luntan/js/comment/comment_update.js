...
$(document).on("click",".edit-btn",function(){
	 ...
	 //2、查询信息，并显示信息  // 坑爹 一开始写成了 attr(edit-id) 
	 getcomment($(this).attr("edit-id"));
	 //3、把id传递给模态框的更新按钮，为了发送ajax时传递id
	 $("#comment_update_done_btn").attr("edit-id",$(this).attr("edit-id"));
	 $("#commentUpdateModal").dialog("open");
	 //alert("edit");
 ...
 function getcomment(id){
   $.ajax({
	 url:"/luntanSSM/comment/get.do?id="+id,
	 type:"get",
	 success:function(result){
	   console.log(result);
	   var commentData = result.extend.comment;
	   $("#postId_update_input").val(commentData.postId);
	   $("#userNum_update_input").val(commentData.userNum);
	   $("#commentContent_update_input").val(commentData.commentContent);
	   $("#update_imgUrl").val(commentData.imgUrl);
	   update_imgName = commentData.update_imgName;
	 }
...
//点击更新，更新
 $("#comment_update_done_btn").click(function(){
 	//查看是否添加了图片
 ...
 //2、发送ajax请求保存更新的数据，保存事件
 function saveData(){
	 $.ajax({
		 url:"/luntanSSM/comment/update.do?id="+$("#comment_update_done_btn").attr("edit-id"),
		 type:"post",
		 data:$("#commentUpdateModal form").serialize()+"&_method=put",
		 success:function(result){
		   $("#commentUpdateModal").dialog("close"); 
