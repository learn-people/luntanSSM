...
//页面加载完成以后，直接发送ajax请求，拿到分页数据
 ...
 function to_page(pn){
   $.ajax({
	 url:"/luntanSSM/fans/selectAll.do",
     data:"pn="+pn,
     type:"GET",
     success:function(result){
       //console.log(result);
       //1、解析并显示员工数据
       build_fans_advert(result);
       //2、解析并显示分页信息
 ...
 * 
 */
 function build_fans_advert(result) {
	 //清空table表格,如果不请空，下一次请求时，数据会在原来的基础上，不断添加。
	 $("#fans_advert tbody").empty();
	 var fans = result.extend.pageInfo.list;
