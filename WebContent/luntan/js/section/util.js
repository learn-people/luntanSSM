  ...
  * 换表时要更改 4-13行的名称
  */
 var g_sectionName_valid = "板块名可用";
 var g_sectionName_invalid_exist = "板块名已存在";
...
//页面加载完成以后，直接发送ajax请求，拿到分页数据
 ...
 function to_page(pn){
   $.ajax({
	 url:"/luntanSSM/section/selectAll.do",
     data:"pn="+pn,
     type:"GET",
 ...
 * 
 */
 function build_section_advert(result) {
	 //清空table表格,如果不请空，下一次请求时，数据会在原来的基础上，不断添加。
	 $("#section_advert tbody").empty();
	 var section = result.extend.pageInfo.list;
	
	 $.each(section, function(index, item) {
		 //alert(item.empName);
		 //构建单元格
		 var checkBoxTd = $("<td><input type='checkbox' class='check-item' /></td>");
		 var sectionIdTd = $("<td></td>").append(item.id);
