/****************************删除信息********************************************************************************************/
 ...
 $(document).on("click",".delete-btn",function(){
   ...
   var sectionName = $(this).parents("tr").find("td:eq(1)").text()
   var id = $(this).attr("delete-id");
   if(confirm("确认删除【"+sectionName+"】")){
     $.ajax({
       url:"/luntanSSM/section/deleteOne.do?id="+id,
       type:"delete",
...
//完成全选、全不选功能
 ...
 //点击全部删除，就是批量删除
 $("#section_delete_all_btn").click(function(){			
 	...
 	$.each($(".check-item:checked"),function(){
 		sectionNames = sectionNames+$(this).parents("tr").find("td:eq(2)").text() +",";
 	    delete_ids_str += $(this).parents("tr").find("td:eq(1)").text() +"-";
 	});
 			
 	//去除sectionNames多余的，
 	sectionNames = sectionNames.substring(0,sectionNames.length-1);
 	//去除删除的id的多余的-
 	delete_ids_str = delete_ids_str.substring(0,delete_ids_str.length-1);
 			
 	if(confirm("确认删除【"+sectionNames+"】?")){
 	//发送ajax请求
 	  $.ajax({
		url:"/luntanSSM/section/delete.do?ids="+delete_ids_str,
		type:"delete",
		success:function(result){
			alert(result.msg);
			to_page(currentPage);
