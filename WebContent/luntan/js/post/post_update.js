...
$(document).on("click",".edit-btn",function(){
	 //0、清空表单样式和内容
	 reset_form("#postUpdateModal form");
 ...
 function getpost(id){
   $.ajax({
	 ...
	 success:function(result){
	   console.log(result);
	   var postData = result.extend.post;
	   $("#userNum_update_input").val(postData.userNum);
	   $("#sectionId_update_select").val(postData.sectionId);
	   $("#postTitle_update_input").val(postData.postTitle);
	   $("#postContent_update_input").val(postData.postContent);
	   $("#purview_update_select").val(postData.purview);
	   $("#update_imgUrl").val(postData.imgUrl);
	   update_imgName = postData.update_imgName;
	 }
...
//点击更新，更新员工信息
 $("#post_update_done_btn").click(function(){
 	//查看是否添加了图片
 ...
 function saveData(){
	 $.ajax({
		 url:"/luntanSSM/post/update.do?id="+$("#post_update_done_btn").attr("edit-id"),
		 type:"post",
		 data:$("#postUpdateModal form").serialize()+"&_method=put",
		 success:function(result){
		   $("#postUpdateModal").dialog("close"); 
